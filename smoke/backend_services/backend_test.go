...
var _ = Describe("BackendServices:", func() {
	...
	Context("Spring Cloud Service Configs service", func() {
		It("can be creaated, used and deleted", func() {
			// Create the service
			...
			Expect(cf.Cf("create-service", "-c", "java_buildpack_offline", "p-config-server standard cook-config-server").Wait(testConfig.GetServiceCreateTimeout())).To(Exit(0))
			// Deploy the app
			Expect(cf.Cf("push", "-b", "java_buildpack_offline", appName, "-p", smoke.SCCJavaAppBitsPath, "-d", testConfig.AppsDomain).Wait(testConfig.GetPushTimeout())).To(Exit(0))
